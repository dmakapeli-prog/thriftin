      ...
      >
        ...
        <div style={{ padding: '15px', flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', marginTop: 'auto' }}>
            <button onClick={handleBeli} style={{ flex: 1, padding: '10px 0', borderRadius: '8px', fontWeight: 600, fontSize: '13px', border: '1.5px solid #7C3AED', color: '#7C3AED', background: 'transparent', cursor: 'pointer' }}>Beli Sekarang</button>
            <button onClick={() => setShowModal(true)} style={{ flex: 1, padding: '10px 0', borderRadius: '8px', fontWeight: 600, fontSize: '13px', background: '#7C3AED', border: '1.5px solid #7C3AED', color: 'white', cursor: 'pointer' }}>Tawar Harga</button>
          </div>
        </div>
      </div>
      ...
      {showModal && (
        <div onClick={() => setShowModal(false)} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: 'white', padding: '30px', borderRadius: '16px', width: '90%', maxWidth: '400px', position: 'relative' }}>
            ...
            <input type="number" placeholder="Misal: 200000" value={offerValue} onChange={(e) => setOfferValue(e.target.value)} style={{ width: '100%', padding: '12px', border: '1px solid #EEEEEE', borderRadius: '8px', outline: 'none', fontFamily: 'inherit', marginTop: '8px', marginBottom: '15px', fontSize: '14px' }} />
            <button onClick={() => { setShowModal(false); setOfferValue(''); alert('Penawaran Anda telah dikirim ke penjual!') }} style={{ width: '100%', padding: '12px', background: '#7C3AED', border: 'none', color: 'white', borderRadius: '8px', fontWeight: 600, fontSize: '14px', cursor: 'pointer' }}>Kirim Penawaran</button>
          </div>
        </div>
      )}
    </>
