...
export default function LiveBanner() {
  ...
  return (
    <>
      <section style={{ backgroundColor: '#E8E0FF', borderRadius: '12px', padding: '15px 25px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '30px', boxShadow: '0 2px 10px rgba(0,0,0,0.03)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
          <div style={{ position: 'relative', width: '60px', height: '60px', borderRadius: '50%', overflow: 'hidden', border: '2px solid #C9B8F0', flexShrink: 0 }}>
          ...
          <div>
            ...
            <h3 style={{ fontSize: '16px', marginBottom: '2px' }}>Thrift Haul: Vintage Collection</h3>
            <p style={{ fontSize: '12px', color: '#FF4D4F', fontWeight: 600 }}><i className="fas fa-eye"></i> 1.2K Menonton</p>
          </div>
        </div>
        <button
          onClick={() => setShowModal(true)}
          style={{ backgroundColor: '#C9B8F0', color: 'white', border: 'none', padding: '12px 30px', borderRadius: '25px', fontSize: '16px', fontWeight: 600, cursor: 'pointer', boxShadow: '0 4px 10px rgba(201,184,240,0.5)', flexShrink: 0 }}
        >Tonton Live</button>
      </section>
      ...
      {showModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => setShowModal(false)}>
          <div style={{ maxWidth: '600px', width: '90%', borderRadius: '16px', overflow: 'hidden', background: '#000', position: 'relative' }} onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setShowModal(false)} style={{ position: 'absolute', top: '15px', right: '15px', background: 'none', border: 'none', color: 'white', fontSize: '20px', zIndex: 10, cursor: 'pointer' }}><i className="fas fa-times"></i></button>
